...
class BankAccount
  ...
  def initialize csvFileData
    @records = csvFileData.map do |line|
      fields = line.split ','
      Transaction.new fields[0], fields[1], fields[2], fields[3].chop
    end
    @records = @records.sort_by { |line| (line.date.split"/")[1].to_i}
  end

  class Transaction
    attr :date, :payee, :amount, :type
  ...
  def summary
    ...
    {start_bal: 0.00, end_bal: end_bal.to_f.round(2), total_d: total_d.to_f.round(2), total_w: total_w.to_f.round(2) }
  end
  ...
  File.open(csvFilename) do |input|
    ...
    File.open("output.txt", "w+") do |output|
      html_block = render_html "Custom Bank Statement" do
        part1 = render_head "Custom Bank Statement"
        part2 = render_body "Bank Info Below:" do
          parta = render_summary december.summary
          partb = render_withdrawals "Withdrawals", december.extract_withdrawal
          partc = render_deposits "Deposits", december.extract_deposit
          parta+partb+partc
        end
        part1+part2
      end
