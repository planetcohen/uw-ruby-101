...
module Assignment08
  class RomanNumeral
    ...
    def to_i
      #The following code is not very pretty.  I'd like to go back
      #and clean this up
      convert = @roman
      last_ten = @roman1.last
      result = @roman1.map.with_index do |c, i|
        unless i == @roman1.length - 1
          if convert =~ /#{@roman1[i]}#{@roman1[i+1]}/
            match = convert[/#{@roman1[i]}#{@roman1[i+1]}/]
            convert = convert.sub(/#{match}/, '')
            9
          elsif convert =~ /#{@roman1[i]}#{@roman5[i]}/
            4
          elsif convert =~ /#{@roman5[i]}/
            if convert =~ /#{@roman1[i]}/
              5 + convert[/#{@roman1[i]}+/].length
            else
              5
            end
          elsif convert =~ /#{@roman1[i]}/
              convert[/#{@roman1[i]}+/].length
          else
            0
          end
        else
  ...
  class TestRoman < Minitest::Test
    #Just wanted to practice reading/writing to a file, so added some gratuitous
    #I/O.  Kind of handy for looking at the Roman numerals that are produced
